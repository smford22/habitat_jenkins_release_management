...
resource "azurerm_virtual_machine" "initial-peer" {
  ...
  provisioner "file" {
    source      = "files/devhosts"
    destination = "/tmp/devhosts"
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo echo -e azure-ubuntu-acceptance >> /etc/hosts",
      "sudo hostnamectl set-hostname azure-ubuntu-acceptance",
    ]
...
resource "azurerm_virtual_machine" "mongodb" {
  ...
  provisioner "file" {
    source      = "files/prodhosts"
    destination = "/tmp/prodhosts"
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo  echo -e azure-ubuntu-production >> /etc/hosts",
      "sudo hostnamectl set-hostname azure-ubuntu-production",
    ]
